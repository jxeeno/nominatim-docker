...
if ${IMPORT_ADMINISTRATIVE}; then
  ...
  osmosis -v \
    ...
    --tf accept-nodes "place=suburb" "boundary=administrative" \
    --tf reject-relations \
    --tf reject-ways \
    --write-pbf file=/srv/nominatim/src/nodes.osm.pbf &
  osmosis -v \
    --read-pbf-fast workers=${FILTER_THREADS} /srv/nominatim/src/data.osm.pbf \
    --tf accept-ways "place=suburb" "boundary=administrative" \
    --tf reject-relations  \
    --used-node \
    --write-pbf file=/srv/nominatim/src/ways.osm.pbf &
  osmosis -v \
    --read-pbf-fast workers=${FILTER_THREADS} /srv/nominatim/src/data.osm.pbf \
    --tf accept-relations "place=suburb" "boundary=administrative" \
    --used-node \
    --used-way \
